...
#define WIFI_STARTUP_TIME_CHECK 3
  ...
  if (sTmr > WIFI_STARTUP_TIME_CHECK && !wStat) {  // check for successful connection
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (!getLocalTime(&timeinfo))
        Serial_print("Failed to get time\n");
      else {
...
void setShortwaveAGC() {  // this will drive an SW attenuator and act as an AGC. SWMinAttn determines the minimum attenuation.
  ...
  static unsigned char currentGain = 0;
  ...
  if (displayDebugInfo && currentGain != newGain) {
