...
#include <ESPAsyncWebServer.h>  // Install from Github: https://github.com/ESP32Async/ESPAsyncWebServer, also install https://github.com/ESP32Async/AsyncTCP
...
void IRAM_ATTR sampleAudio() {  //audio sampler, 8bit unsigned every 125 uS into frame[].
  ...
  if (frameIndex >= currAudioBufSize) {  //currAudioBufSize changes btw 255 and 4095. bigger reduces clicking caused by scriptprocessor. smaller reduces audio lag.
    frameIndex = 0;
...
void loop() {
  ...
  if (!streamAudio) {  // streaming audio needs all available resources,  shorten main loop when active
    ...
    if (fTrigger % 8 == 0 && !disableFFT && !webIFActive) {


      audioSpectrum();  // spectrum eats a lot of processing resources
...
void displaySTEP(bool update) {
  ...
  const uint16_t xPos = 340;
  const uint16_t yPos = 26;
  ...
  else if (STEP >= 1000 && STEP < 1000000)
...
*/
  ...
  const int hStart = 8;
  const int ySpace = 57;   //57
  const int yStart = 121;  //121
  ...
  if (sTmr > WIFI_STARTUP_TIME && !wFinished && ssid.length() > 0) {  // SSID is configured, connect and check connection
    ...
    if (WiFi.status() != WL_CONNECTED) {
      ...
      while (!getLocalTime(&timeinfo) && retries < 5) {
        ...
        retries++;
        Serial.printf("getLocalTime retries: %d\n", retries);
      }
      ...
      if (retries < 5) {
        ...
        Serial.println("Failed to get time,timeout");
      }
      wFinished = true;
    }
...
void indicatorTouch() {  // Change parameters by touching the indicator areas
  ...
  if (ty > 296 && tx <= 130) {  // debug info on/off
    ...
    if (!displayDebugInfo)
      tft.fillRect(143, 140, 18, 25, TFT_BLACK);  // agc gain
...
void sineTone(uint16_t frequency, uint16_t duration) {
...
void setAFC(int afcVoltage) {  // only for NBFM hardware, generates AFC voltage and uses the SI5351 oscillator correction to tune to frequency
  ...
  si5351.set_correction(SI5351calib + shift, SI5351_PLL_INPUT_XO);  // use SI5351correction instead of adjusting FREQ to avoid feedback loop
}
...
void setShortwaveAGC(bool show) {  // this will drive an SW attenuator and act as an AGC. SWMinAttn determines the minimum fixed attenuation.
  ...
  if (displayDebugInfo && show) {

    etft.setTTFFont(Arial_14);
    etft.setTextColor(redToGreen(SWAttn));
    ...
    if (lastSignalStrength != signalStrength) {  // overwrite old and write new vale
      tft.setTextSize(1);
      tft.setTextColor(TFT_BLACK);
      tft.setCursor(145, 135);
      tft.printf("%d", lastSignalStrength);
      tft.setTextColor(TFT_WHITE);
      tft.setCursor(145, 135);
...
void checkRFModeChange(void) {  // check whether FREQ has moved from SW to VHF/UHF or viceversa;
  ...
  if ((FREQ <= SHORTWAVE_MODE_UPPER_LIMIT && FREQ_OLD >= SHORTWAVE_MODE_UPPER_LIMIT) || (FREQ >= SHORTWAVE_MODE_UPPER_LIMIT && FREQ_OLD <= SHORTWAVE_MODE_UPPER_LIMIT)) {
    ...
    // a switchover has occured, this is needed to refresh the attenuator button label
...
void streamAudioLoop() {  //reduced loop that runs while streaming audio.
  ...
  if (bufferFilled) {
    ...
    if (streamAudio) {
      ...
      if (currAudioBufSize < frSize) {  // freq or mode has changed
        for (int i = 0; i < frSize; i++)
          frame[i] = 0;                         //eliminate remains of last fuffer
        ws.binaryAll(frame, currAudioBufSize);  // short silence
        currAudioBufSize = frSize;
      }

      else
        ws.binaryAll(frame, currAudioBufSize);  // push audio
    }
