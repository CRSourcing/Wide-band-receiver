...
1. RF gets amplified 20dB using a MAR-6 and a SBA4089 gain block. YG602020 or any other gain block that goes down to 100KHz with a high enough IP3 can also be used. 
A small signal relay switches the LNA output betwee the tuner input (75Ohms) and the 0-50MHz lowpass filter for SW mode.
...
If the SD card option does not work, you can use the LittleFS uploader (https://github.com/earlephilhower/arduino-littlefs-upload) to upload the files to the file system.
...
17. For better frequency stability a TCXO can be used: Add a 25MHz TCXO to SI5351 module as described on the internet. 
